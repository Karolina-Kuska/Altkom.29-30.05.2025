...
namespace ConsoleApp
{
    internal class Transactions
    {
        ...
        public static void Run(DbContextOptionsBuilder<Context> config, bool randomFail = true)
        {
            ...
            var orders = Enumerable.Range(1, 5).Select(x => new Order { Tax = Random.Shared.NextSingle(), Value = Random.Shared.Next(100), Name = $"Zamówienie #{x}", OrderDate = DateTime.Now.AddDays(3.21 * x), OrderType = (OrderType)(x % 3), Parameters = (Parameters)(x % 127) }).ToList();
